...
CAN_IPC_RECEIVER_FILTER_T* CAN_IPC_RECEIVER::return_can_ipc_receiver_filter(const CAN_DEV_PORT_E can_port)
{
	switch(can_port){
...
int CAN_IPC_RECEIVER::register_can_filter(const CAN_IPC_FILTER_T &can_ipc_filter)
{
	...
	CAN_IPC_RECEIVER_FILTER_T *can_ipc_receiver_filter = this->return_can_ipc_receiver_filter(static_cast<CAN_DEV_PORT_E>(can_ipc_filter.can_port));
	if(can_ipc_receiver_filter == nullptr){
		return -RET_CODE_INVALID_ARG;
	}

	auto filter_result = can_ipc_receiver_filter->can_ipc_filter_map.emplace(can_ipc_filter.can_filter_id,
							      can_ipc_filter);
	...
	auto id_result =
		can_ipc_receiver_filter->can_ipc_filter_id_map.emplace(can_ipc_filter.can_filter_mask, 1);
	LOG_DEBUG("id_result.mask: "
		  << static_cast<uint32_t>(can_ipc_filter.can_filter_mask));
	LOG_DEBUG("id_result.count: " << static_cast<uint32_t>(
			  can_ipc_receiver_filter->can_ipc_filter_id_map[can_ipc_filter.can_filter_mask]));
	...
	auto mask_result =
		can_ipc_receiver_filter->can_ipc_filter_mask_map.emplace(can_ipc_filter.can_filter_mask, 1);
	LOG_DEBUG("mask_result.mask: "
		  << static_cast<uint32_t>(can_ipc_filter.can_filter_mask));
	LOG_DEBUG("mask_result.count: " << static_cast<uint32_t>(
			  can_ipc_receiver_filter->can_ipc_filter_mask_map[can_ipc_filter.can_filter_mask]));
	// if return false, means this can id already exist, so increase the count value by 1
...
int CAN_IPC_RECEIVER::deregister_can_filter(
	const CAN_IPC_FILTER_T &can_ipc_filter)
{
	...
	auto mask_it = can_ipc_receiver_filter->can_ipc_filter_mask_map.find(can_ipc_filter.can_filter_mask);
	if ((mask_it != can_ipc_receiver_filter->can_ipc_filter_mask_map.end()) && (mask_it->second != 0)) {
		mask_it->second -= 1;
	} else if ((mask_it != can_ipc_receiver_filter->can_ipc_filter_mask_map.end()) && (mask_it->second == 0)) {
		can_ipc_receiver_filter->can_ipc_filter_mask_map.erase(can_ipc_filter.can_filter_mask);
	...
	auto id_it = can_ipc_receiver_filter->can_ipc_filter_id_map.find(can_ipc_filter.can_filter_id);
	if ((id_it != can_ipc_receiver_filter->can_ipc_filter_id_map.end()) && (id_it->second != 0)) {
		id_it->second -= 1;
	}

	if ((id_it != can_ipc_receiver_filter->can_ipc_filter_id_map.end()) && (id_it->second == 0)) {
		auto filter_it = can_ipc_receiver_filter->can_ipc_filter_map.find(can_ipc_filter.can_filter_id);
		if (filter_it != can_ipc_receiver_filter->can_ipc_filter_map.end()) {
			can_ipc_receiver_filter->can_ipc_filter_map.erase(can_ipc_filter.can_filter_id);
			can_ipc_receiver_filter->can_ipc_filter_id_map.erase(can_ipc_filter.can_filter_id);
